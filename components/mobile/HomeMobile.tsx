...
export default function HomeMobile() {
  ...
  return (
    <main style={{ background: 'var(--bg)', overflowX: 'hidden' }}>
      ...
      <section style={{
        ...
        <div style={{ position: 'absolute', top: '36%', left: 0, right: 0, padding: '0 28px', transform: 'translateY(-50%)' }}>
          <h1 style={{ margin: 0, lineHeight: 1.05 }}>
            <span style={{ display: 'block', fontSize: 'clamp(52px,14vw,76px)', fontWeight: 500, color: '#fff', letterSpacing: '0.04em', fontFamily: 'var(--font-cormorant), Cormorant Garamond, serif', fontStyle: 'italic' }}>CORTINAS</span>
            <span style={{ display: 'block', fontSize: 'clamp(12px,3.2vw,16px)', fontWeight: 500, color: 'rgba(255,255,255,0.9)', letterSpacing: '0.22em', marginTop: 10, fontFamily: 'var(--font-cormorant), Cormorant Garamond, serif' }}>
              QUE TRANSFORMAN TUS <span style={{ color: '#6B8CFF' }}>ESPACIOS</span>
            </span>
          </h1>
        </div>

        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '0 20px 32px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          <Link href="/configurador" style={{ textDecoration: 'none' }}>
            <div style={{ background: 'rgba(250,248,245,0.97)', borderRadius: 12, padding: '13px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 4px 24px rgba(0,0,0,0.2)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
                <div style={{ color: '#14008C' }}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg></div>
                <span style={{ fontSize: 12, fontWeight: 800, letterSpacing: '0.08em', color: '#0D0D0D' }}>CORTINAS A MEDIDA</span>
              </div>
              <span style={{ fontSize: 15, color: '#14008C', fontWeight: 700 }}>→</span>
            </div>
          </Link>
          <Link href="/stock" style={{ textDecoration: 'none' }}>
            <div style={{ background: 'rgba(20,0,140,0.88)', borderRadius: 12, padding: '13px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 4px 24px rgba(0,0,0,0.2)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
                <div style={{ color: '#fff' }}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg></div>
                <span style={{ fontSize: 12, fontWeight: 800, letterSpacing: '0.08em', color: '#fff' }}>LISTAS PARA LLEVAR</span>
              </div>
              <span style={{ fontSize: 15, color: '#fff', fontWeight: 700 }}>→</span>
            </div>
          </Link>
        </div>
      </section>
      ...
      {/* ── TICKER ── */}
      <div style={{ background: '#F5F0E8', overflow: 'hidden', height: 44, display: 'flex', alignItems: 'center', borderTop: '1px solid #E8E0D4', borderBottom: '1px solid #E8E0D4' }}>
            ...
            <span key={rep} style={{ display: 'flex', alignItems: 'center' }}>
              {['3 Y 6 CUOTAS SIN INTERÉS', '30% DE DESCUENTO EN EFECTIVO O TRANSFERENCIA'].map((text, i) => (
                <span key={i} style={{ display: 'inline-flex', alignItems: 'center' }}>
                  <span style={{ color: '#0A0A14', fontWeight: 600, letterSpacing: '0.15em', padding: '0 28px', fontFamily: 'var(--font-cormorant), Cormorant Garamond, serif', fontSize: 12 }}>{text}</span>
      ...
      <section style={{ padding: '40px 20px 0' }}>
        ...
        <h2 style={{ fontSize: 26, fontWeight: 800, letterSpacing: '-0.02em', color: 'var(--text)', margin: '0 0 20px 0' }}>Descubrí ambientes únicos</h2>
            ...
            <div key={a.key} onClick={() => abrirModal(a)} style={{ borderRadius: 14, overflow: 'hidden', border: '1px solid var(--border)', background: 'var(--surface)', cursor: 'pointer' }}>
              ...
              <div style={{ padding: '10px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                  ...
                  <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.1em', color: '#0A0A14' }}>{a.label}</span>
                </div>
                <span style={{ color: '#14008C', fontWeight: 700, fontSize: 13 }}>→</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* ── GUÍAS 2x2 ── */}
      <section style={{ padding: '40px 20px 0' }}>
        <h2 style={{ fontSize: 26, fontWeight: 800, letterSpacing: '-0.02em', color: 'var(--text)', margin: '0 0 20px 0' }}>Todo lo que necesitás</h2>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {ACCESOS.map(a => (
            <Link key={a.titulo} href={a.href} style={{ textDecoration: 'none' }}>
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '16px 14px', display: 'flex', flexDirection: 'column', height: '100%', boxSizing: 'border-box' }}>
                ...
                <div style={{ fontSize: 10, fontWeight: 700, color: '#14008C', letterSpacing: '0.12em', marginBottom: 5 }}>{a.num}</div>
                <div style={{ fontSize: 13, fontWeight: 800, color: '#0A0A14', marginBottom: 6, lineHeight: 1.3 }}>{a.titulo}</div>
                <p style={{ fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.5, margin: '0 0 12px 0', flex: 1 }}>{a.desc}</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, borderTop: '1px solid var(--border)', paddingTop: 10 }}>
                  <span style={{ fontSize: 10, fontWeight: 800, color: '#14008C', letterSpacing: '0.08em' }}>{a.label}</span>
                  <span style={{ color: '#14008C', fontWeight: 700, fontSize: 11 }}>→</span>
                </div>
              </div>
            </Link>
          ))}
      ...
      <footer style={{ background: '#E8E2D9', color: '#0A0A14', padding: '40px 20px 48px', marginTop: 40 }}>
        <div style={{ marginBottom: 28 }}>
          <Image src="/images/LOGO.png" alt="Insumos Roller" width={140} height={52} style={{ objectFit: 'contain', height: 40, width: 'auto' }} />
          <p style={{ fontSize: 13, color: '#8888A8', lineHeight: 1.7, margin: '10px 0 0 0' }}>Fabricación a medida. Envíos a todo el país.</p>
        </div>
        <div style={{ borderTop: '1px solid rgba(0,0,0,0.08)' }}>
          {Object.entries(FOOTER_LINKS).map(([titulo, links]) => (
            <div key={titulo} style={{ borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
              <button onClick={() => setFooterAbierto(footerAbierto === titulo ? null : titulo)} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 0', background: 'none', border: 'none', cursor: 'pointer' }}>
                <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', color: '#555', textTransform: 'uppercase' }}>{titulo}</span>
                <span style={{ color: '#888', fontSize: 18, display: 'inline-block', transition: 'transform 0.2s', transform: footerAbierto === titulo ? 'rotate(45deg)' : 'none' }}>+</span>
              </button>
        ...
        <p style={{ fontSize: 12, color: '#8888A8', margin: '28px 0 0 0', textAlign: 'center' }}>© 2025 Insumos Roller. Todos los derechos reservados.</p>
      </footer>
          ...
          <div onClick={e => e.stopPropagation()} style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '20px 16px 16px' }}>
            ...
            <div style={{ display: 'flex', gap: 6, overflowX: 'auto', padding: '12px 0 0', scrollbarWidth: 'none' }}>
              {ambienteActivo.fotos.map((foto, i) => (
                <div key={i} onClick={() => setFotoActiva(i)} style={{ position: 'relative', flexShrink: 0, width: 60, height: 40, borderRadius: 6, overflow: 'hidden', cursor: 'pointer', border: i === fotoActiva ? '2px solid #fff' : '2px solid transparent', opacity: i === fotoActiva ? 1 : 0.45 }}>
