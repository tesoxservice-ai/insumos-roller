...
export async function generarPDF(items: ItemPresupuesto[]): Promise<void> {
  // Importación dinámica para evitar SSR issues con jsPDF
  ...
  doc.setFontSize(9)
  doc.setTextColor(184, 173, 158) // --text-mid
  ...
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(14)
  doc.setTextColor(242, 237, 228) // --text
  ...
  // Encabezado de tabla
  doc.setFillColor(26, 24, 20) // --surface
  ...
  for (const [i, item] of items.entries()) {
    ...
    const bgColor = i % 2 === 0 ? [34, 31, 26] : [26, 24, 20] // surface2 / surface
  ...
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(8)
  doc.setTextColor(122, 114, 105) // --text-muted
  doc.text(
    'Este presupuesto tiene vigencia de 48 horas hábiles.',
    margen,
    y
  )
